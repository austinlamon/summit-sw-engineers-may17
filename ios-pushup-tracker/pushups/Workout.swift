...
class Workout{
    ...
    init (name: String, startDate: Date, endDate: Date, pushupsCompleted: Int) {
        self.name = name
    ...
    func elapsedTime() -> (minutes: Int,seconds: Int) {
        let seconds =  (Calendar.current as NSCalendar).components(.second, from: startDate, to: endDate, options: []).second
        if(seconds! < 60){
            return(0,seconds!)
        }
        else{
